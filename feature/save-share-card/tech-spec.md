...
| 2026-06-09 | Grace Gettert | **Retrospective (Learn phase).** Built as designed — no architectural surprises. Two deviations from plan: (1) test script changed to `node --test test/*.test.js` (not bare `node --test`) to avoid `node --test` following the broken `.claude/skills` symlink in CI; (2) canvas renderer reuses the inline win-line set from `index.html` rather than importing `isWinningCard` — `src/bingo.js` is not HTTP-served and would 404 in the browser. Both were low-risk; neither required rework after Build. ADRs: [0001](../../adr/0001-client-only-persistence-via-localstorage.md), [0002](../../adr/0002-pure-client-module-in-public-dir.md), [0003](../../adr/0003-canvas-png-for-share-image.md). |
